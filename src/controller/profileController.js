...
export const getUsersWithProfileImage = async (req, res) => {
  try {
    ...
    const photo = await RegisterModel.find({
      ...
      'vmId firstName lastName dateOfBirth height religion caste employedIn ' +
...
export const getNewlyRegisteredUsers = async (req, res) => {
  try {
    ...
    const currentUser = await RegisterModel.findById(currentUserId).select('gender');
    ...
    if (currentUser.gender === 'Male') {
      ...
      oppositeGender = { $ne: currentUser.gender };
    }
    ...
    const requestBlockedIds = allRequests.map(req => {
      return req.sender.toString() === currentUserId.toString()
    ...
    const users = await RegisterModel.find({
      ...
      '_id vmId firstName lastName dateOfBirth height religion caste employedIn ' +
      'annualIncome highestEducation currentCity city state currentState ' +
      'motherTongue gender profileImage updatedAt createdAt designation'
    );
