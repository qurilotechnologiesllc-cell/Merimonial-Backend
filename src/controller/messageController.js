...
export const getAllRequestsExceptMine = async (req, res) => {
  try {
    ...
    const requests = await AccountRequestModel.find({
      status: "accepted",
      $or: [{ sender: { $ne: userId } }, { receiverId: { $ne: userId } }]
    })
    ...
    const otherUsers = requests
      .map((r) => {
        if (!r?.sender || !r?.receiverId) return null;
        const otherUser = String(r.sender._id) === String(userId)
          ? r.receiverId
          : r.sender;
...
export const getChatList = async (req, res) => {
  try {
    ...
    const relationships = await AccountRequestModel.find({
      ...
      .populate("sender", "firstName lastName profileImage blockedUsers")
      .populate("receiverId", "firstName lastName profileImage blockedUsers");

    let finalUsers = [];

    for (const reqDoc of relationships) {
      if (!reqDoc.sender || !reqDoc.receiverId) continue;

      const other = String(reqDoc.sender._id) === String(userId)
        ? reqDoc.receiverId
        : reqDoc.sender;

      const otherBlocked = other.blockedUsers?.includes(userId);
      const currentUser = String(reqDoc.sender._id) === String(userId)
        ? reqDoc.sender
        : reqDoc.receiverId;
...
export const getOnlineStatus = async (req, res) => {
  try {
    ...
    // console.log(userId, onlineUserIds);
...
export const getAllUser = async (req, res) => {
  try {
    ...
    const users = acceptedRequests
      .map(reqDoc => {
        if (!reqDoc?.sender || !reqDoc?.receiverId) return null;
        return String(reqDoc.sender._id) === String(userId)
          ? reqDoc.receiverId
          : reqDoc.sender;
      })
      .filter(Boolean);
...
export const blockUser = async (req, res) => {
  try {
    ...
    const response = await RegisterModel.findByIdAndUpdate(
      userId,
      {
